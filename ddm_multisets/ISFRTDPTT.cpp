...
int ISFfun(const gsl_vector* para, void* sdata, gsl_vector* y)
{
	...
	const int cnum_qCurve_low = ((dataStruct *)sdata)->num_qCurve_low;
	const int cnum_qCurve_high = ((dataStruct *)sdata)->num_qCurve_high;
    //Get the total number of data points
	const int tnum_fit = num_fit*(cnum_qCurve_low + cnum_qCurve_high);
    //Get the parameters.
	...
	for (int iterqc = 0; iterqc<cnum_qCurve_low; ++iterqc)
	{
	...
	for (int iterqc = cnum_qCurve_low; iterqc<cnum_qCurve_high+cnum_qCurve_low; ++iterqc)
	{
		//Each curve have different value of q, but A(q) and B(q) depend on q.
		const long double A = gsl_vector_get(para, 6 + 2 * iterqc);
		const long double B = gsl_vector_get(para, 7 + 2 * iterqc);
...
int dISFfun(const gsl_vector* para, void* sdata, gsl_matrix* J)
{
    ...
    if (TT<TTLowBound)
    {
        for (int iter=0; iter<tnum_fit; ++iter)
        {
            gsl_matrix_set(J, iter, 5, 2e5*(TT-TTLowBound));
        }
        breakFlag=true;
    }
